...
import { CalendarDays, Euro, 
  TrendingUp, Clock, CheckCircle, Bell, Package, MessageSquare, Eye, Trophy
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const { data: competitionEntries } = useQuery({
    ...
    queryFn: () => base44.entities.CompetitionEntry.list('-created_date', 100),
    initialData: []
  });
  ...
  const pendingCompetitionEntries = competitionEntries.filter(e => e.status === 'pendente');
  ...
  const stats = [
    ...
    { 
      ...
      value: pendingCompetitionEntries.length, 
      icon: Trophy, 
      color: 'bg-stone-800',
      bgLight: 'bg-stone-50',
      link: 'AdminCompetitionEntries'
    },
  ];
  ...
  return (
    <AdminLayout currentPage="AdminDashboard">
      <div className="space-y-6">
        ...
        {(pendingBookings.length > 0 || pendingOrders.length > 0 || messages.length > 0 || pendingCompetitionEntries.length > 0) && (
          <motion.div
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-3 mt-1 text-sm">
                  ...
                  {pendingCompetitionEntries.length > 0 && (
                    <>
                      {(pendingBookings.length > 0 || pendingOrders.length > 0) && <span>•</span>}
                      <Link to={createPageUrl('AdminCompetitionEntries')} className="hover:underline">
                        {pendingCompetitionEntries.length} inscriç{pendingCompetitionEntries.length > 1 ? 'ões' : 'ão'} pendente{pendingCompetitionEntries.length > 1 ? 's' : ''}
                      </Link>
                    </>
                  )}
                  {messages.length > 0 && (
