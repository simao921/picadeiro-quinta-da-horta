...
export default function PaymentsSection({ user }) {
  ...
  const getStatusBadge = (status) => {
    const config = {
      ...
      overdue: { label: 'Em Atraso', class: 'bg-red-100 text-red-800', icon: AlertTriangle },
  ...
  return (
    <div className="space-y-6">
      ...
      {isBlocked && (
        <div className="p-4 bg-red-50 border-2 border-red-200 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-800 mb-1">⚠️ Conta Bloqueada</h3>
              ...
              <div className="text-sm text-red-600">
                ...
                <ul className="list-disc ml-5 mt-1">
                  <li>Não pode fazer novas reservas</li>
                  <li>Não pode participar em aulas</li>
                  <li>Não pode participar em provas</li>
                  <li>Não pode representar o clube</li>
                </ul>
              </div>
      ...
      {totalDebt > 0 && !isBlocked && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-amber-800">Pagamentos Pendentes</h3>
              <p className="text-amber-700 text-sm mt-1">
                Por favor regularize os seus pagamentos para evitar penalizações.
              </p>
              <p className="text-amber-800 font-semibold mt-2">
                Valor em dívida: {totalDebt.toFixed(2)}€
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-0 shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-stone-500">Total Pago</p>
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-stone-500">Pendente</p>
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-stone-500">Penalizações</p>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold text-[#2C3E1F]">
                          Mensalidade {payment.month}
                        </span>
                        {getStatusBadge(payment.status)}
                      </div>
                      <div className="text-sm text-stone-600 space-y-1">
                        <p>Vencimento: {payment.due_date ? format(new Date(payment.due_date), "d 'de' MMMM", { locale: pt }) : 'N/A'}</p>
                        {payment.paid_at && (
                          <p className="text-green-600">
                            Pago em: {format(new Date(payment.paid_at), "d 'de' MMMM", { locale: pt })}
                          </p>
                        )}
                        {payment.penalty > 0 && (
                          <p className="text-red-600 font-medium">
                            Penalização aplicada: +{payment.penalty.toFixed(2)}€
                          </p>
                        )}
                      </div>
                    </div>
      ...
      <Card className="border-0 shadow-md bg-stone-50">
        <CardHeader>
          <CardTitle className="text-lg">Regras de Pagamento</CardTitle>
        </CardHeader>
        <CardContent>
